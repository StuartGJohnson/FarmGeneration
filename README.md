...
This orchard is not usable as is in IsaacSim due to the large number of vertices and faces in the ground-cover assets. This completely stalls the simulation. One answer is to generate all assets procedurally in python code.

An IsaacSim render of a similar orchard with assets generated by `tests/test_generate_assets.py` -> `test_output/orchard_generated_trees_world.usda` suggests that one can procedurally generate usable assets:
